...
export default async function DashboardPage() {
  ...
  return (
    <div className="page-wrap dashboard-page">
      <PageHeading
        description="这里是你的中转服务概况：密钥数量、连接状态，以及它是怎么工作的。"
        title="总览"
      />
      ...
      <section aria-label="服务状态" className="stat-grid">
        <article className="stat-card surface">
          <span>有效密钥</span>
          <strong>{activeKeys} 把</strong>
          <Link className="stat-link" href="/keys">管理密钥</Link>
        </article>
        <article className="stat-card surface">
          <span>数据库</span>
      ...
      <section aria-label="隐私承诺" className="privacy-grid">
        ...
        <article className="privacy-card">
          ...
          <h3>密钥受保护</h3>
          <p>OpenRouter 真实密钥只放在服务器上，不会出现在你的设备里。</p>
        </article>
      </section>
      ...
      <section className="next-step surface">
        <div>
          <h2>接下来做什么？</h2>
          <p>为设备创建一把密钥，或者查看接入地址完成软件设置。</p>
        </div>
        <div className="next-step-actions">
          <Link className="button button-primary" href="/keys">创建密钥</Link>
